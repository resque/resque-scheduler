...
module Resque
  module Scheduler
    module DelayingExtensions
      ...
      def enqueue_at(timestamp, class_type, *args)
        validate(class_type)
        enqueue_at_with_queue(
          queue_from_class(class_type), timestamp, class_type, *args
        )
      end
      ...
      def enqueue_at_with_queue(queue, timestamp, class_type, *args)
        return false unless plugin.run_before_schedule_hooks(class_type, *args)
        ...
        if Resque.inline? || timestamp.to_i < Time.now.to_i
          ...
          if class_type.respond_to?(:scheduled)
            class_type.scheduled(queue, class_type.to_s, *args)
          else
            Resque::Job.create(queue, class_type, *args)
          end
        else
          delayed_push(timestamp, job_to_hash_with_queue(queue, class_type, args))
        end

        plugin.run_after_schedule_hooks(class_type, *args)
      end

      # Identical to enqueue_at but takes number_of_seconds_from_now
      # instead of a timestamp.
      def enqueue_in(number_of_seconds_from_now, class_type, *args)
        enqueue_at(Time.now + number_of_seconds_from_now, class_type, *args)
      end
      ...
      def remove_delayed(class_type, *args)
        search = encode(job_to_hash(class_type, args))
        remove_delayed_job(search)
      end

      # Given an encoded item, enqueue it now
      def enqueue_delayed(class_type, *args)
        hash = job_to_hash(class_type, args)
        remove_delayed(class_type, *args).times do
      ...
      # certain criteria
      def remove_delayed_selection(class_type = nil)
      ...
      # certain criteria
      def enqueue_delayed_selection(class_type = nil)
        fail ArgumentError, 'Please supply a block' unless block_given?
      ...
      # certain criteria
      def find_delayed_selection(class_type = nil, &block)
        fail ArgumentError, 'Please supply a block' unless block_given?
        ...
        while start = search_first_delayed_timestamp_in_range(start, nil)
          ...
          while index >= 0
            ...
            if payload_matches_selection?(decoded_payload, class_type, &block)
      ...
      # Given a timestamp and job (class_type + args) it removes all instances and
      # returns the count of jobs removed.
      #
      # O(N) where N is the number of jobs scheduled to fire at the given
      # timestamp
      def remove_delayed_job_from_timestamp(timestamp, class_type, *args)
        return 0 if Resque.inline?

        key = "delayed:#{timestamp.to_i}"
        encoded_job = encode(job_to_hash(class_type, args))
      ...
      def delayed?(class_type, *args)
        !scheduled_at(class_type, *args).empty?
      end

      # Returns delayed jobs schedule timestamp for +class_type+, +args+.
      def scheduled_at(class_type, *args)
        search = encode(job_to_hash(class_type, args))
      ...
      def job_to_hash(class_type, args)
        { class: class_type.to_s, args: args, queue: queue_from_class(class_type) }
      end

      def job_to_hash_with_queue(queue, class_type, args)
        { class: class_type.to_s, args: args, queue: queue }
      end
      ...
      def payload_matches_selection?(decoded_payload, class_type)
        return false if decoded_payload.nil?
        job_class = decoded_payload['class']
        relevant_class = (class_type.nil? || class_type.to_s == job_class)

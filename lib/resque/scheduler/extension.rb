    ...
    module Extension
      ...
      def enqueue_at_with_queue(queue, timestamp, klass, *args)
        ...
        if Resque.inline? || timestamp.to_i < Time.now.to_i
          ...
          # inline.  If the class is a custom job class, call self#scheduled on
          # it. This allows you to do things like Resque.enqueue_at(timestamp,
      ...
      # +timestamp+ can be either in seconds or a datetime object Insertion if
      # O(log(n)).  Returns true if it's the first job to be scheduled at that
      # time, else false
      ...
      def remove_delayed_selection
        ...
        destroyed = 0
        # There is no way to search Redis list entries for a partial match, so
        # we query for all delayed job tasks and do our matching after decoding
        # the payload data

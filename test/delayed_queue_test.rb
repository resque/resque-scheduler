...
context "DelayedQueue" do
  ...
  test "remove_delayed_with_queue removes job and returns the count" do
    t = Time.now + 120
    Resque.enqueue_at_with_queue('foo', t, SomeIvarJob)

    assert_equal(1, Resque.remove_delayed_with_queue('foo', SomeIvarJob))
  end
  ...
  test "remove_delayed_job_from_timestamp_with_queue removes instances of jobs at a given timestamp" do
    t = Time.now + 120
    Resque.enqueue_at_with_queue('foo', t, SomeIvarJob, "foo")
    assert_equal 1, Resque.remove_delayed_job_from_timestamp_with_queue('foo', t, SomeIvarJob, "foo")
    assert_equal 0, Resque.delayed_timestamp_size(t)
  end

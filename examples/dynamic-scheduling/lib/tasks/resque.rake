...
namespace :resque do
  task setup: :environment do
    ...
    # In resque-only servers we must require each job class individually, because we're not
    # running the full Rails app
    require "#{rails_root}/app/jobs/send_email_job"
    require "#{rails_root}/app/jobs/fix_schedules_job"

    resque_config = YAML.load_file File.join(rails_root.to_s + 'config', 'resque.yml')

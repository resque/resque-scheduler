...
# when restarting. This means all dynamic schedules, which are 
# added via the API, are wiped on each application redeployment. A workaround for
# this sometimes undesirable behavior is to make this job part of a
...
# and will add back the dynamic schedules that were wiped on restart. It
...
class FixSchedulesJob
  ...
  ##
  # Fix email sending schedules. Any user which does not have scheduled sending of emails
  # will be detected, and the missing scheduled job will be added to resque-schedule.
  #
